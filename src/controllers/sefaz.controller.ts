...
function assertDistribuicaoBody(body: unknown): DistribuicaoRequestBody {
  if (!body || typeof body !== 'object') {
    throw new AppError('JSON inválido', { statusCode: 400, code: 'INVALID_JSON', expose: true });
  }
  ...
  if (!/^\d{14}$/.test(cnpj)) {
    throw new AppError('cnpj inválido (14 dígitos)', { statusCode: 400, code: 'INVALID_CNPJ', expose: true });
  }

  const cUF = String(b.cUF ?? '').trim();
  if (!/^\d{2}$/.test(cUF)) {
    throw new AppError('cUF inválido (2 dígitos)', { statusCode: 400, code: 'INVALID_CUF', expose: true });
  }

  const tpAmb = String(b.tpAmb ?? '').trim();
  if (!/^[12]$/.test(tpAmb)) {
    throw new AppError('tpAmb inválido (1 ou 2)', { statusCode: 400, code: 'INVALID_TPAMB', expose: true });
  }

  const ultNSU = String(b.ultNSU ?? '').trim();
  if (!/^\d{15}$/.test(ultNSU)) {
    throw new AppError('ultNSU inválido (15 dígitos)', { statusCode: 400, code: 'INVALID_ULTNSU', expose: true });
  }
...
export async function postDistribuicao(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  ...
  const result = await consultarDistribuicao({ body, logger: request.log });
